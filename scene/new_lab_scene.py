    ...
    def run(self):
        ...
        while not scene_exit:
            ...
            if Key_down:
                ...
                if self.professor.stop and self.researcher.pick_letter == 2 and pressed_key == K_y:
                    self.fade.set_status(SceneStatus.Out)
                    self.professor.stop = False
                    self.scene_result = SceneResult.Win
                elif self.professor.stop and self.researcher.pick_letter != 2 and pressed_key == K_y:
                    self.fade.set_status(SceneStatus.Out)
                    self.professor.stop = False
                    self.scene_result = SceneResult.Fail
